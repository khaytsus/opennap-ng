...
$Id: debug.c 434 2006-09-03 17:48:47Z reech $ */
...
static int debug_hash(void *ptr)
{
    int     hash;
...
static int debug_overflow(BLOCK * block, const char *func)
{
    if(*((unsigned char *) block->val + block->len) != END_BYTE)
    {
        fprintf(stderr, "debug_%s: buffer overflow detected in data allocated at %s:%d\n", func, block->file, block->line);
        return 1;
    }
    return 0;
}

static void debug_exhausted(const char *file, int line)
{
...
void   *debug_realloc(void *ptr, int bytes, const char *file, int line)
{
    ...
    if(ptr)
    {
        ...
        if(!block)
        {
            fprintf(stderr, "debug_realloc(): invalid pointer at %s:%d\n", file, line);
            return 0;
        }
...
void debug_free(void *ptr, const char *file, int line)
{
    ...
    if(!ptr)
    {
        fprintf(stderr, "debug_free: attempt to free NULL pointer at %s:%d\n", file, line);
        return;
    }
    offset = debug_hash(ptr);
    if(!Allocation[offset])
    {
        fprintf(stderr, "debug_free: attempt to free bogus pointer at %s:%d\n", file, line);
        return;
    }
...
int debug_valid(void *ptr, int len)
{
    BLOCK  *block;
    int     offset;

    offset = debug_hash(ptr);
    for (block = Allocation[offset]; block && (char *) ptr > (char *) block->val + block->len; block = block->next);
    if(!block)
    {
